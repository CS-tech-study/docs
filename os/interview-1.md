...
<img width="478" alt="스크린샷 2025-02-27 오전 1 18 59" src="https://github.com/user-attachments/assets/8203a959-22f1-4fcf-a849-4a22fe76b40a" />


### 메모리 모델

여러 스레드가 메모리에 접근할 때 발생할 수 있는 동기화 문제와 메모리 일관성을 보장하기 위한 **추상적인 규칙 집합**
>OS는 bus snooping과 같은 물리적인 매커니즘으로 캐시 일관성을 유지하는 반면, JVM은 플랫폼 독립적이어야 하므로 이를 작업 메모리, 메인 메모리라는 추상적인 모델을 사용

<img width="348" alt="스크린샷 2025-02-27 오전 1 21 41" src="https://github.com/user-attachments/assets/8d579a1c-1d49-495e-9b6f-541e15ca8eb0" />


메인 메모리가 하드웨어 메모리에 대응되고, 작업 메모리가 레지스터와 캐시에 대응
>작업 메모리를 실제로 레지스터나 캐시에 미리 저장해둔다고 함

- 작업 메모리에는 메인 메모리의 복사본이 저장
- 변수를 읽고 쓰는 모든 연산은 작업 메모리에서 수행되며, 메인 메모리의 데이터는 직접 읽고 쓸 수 없음

## 자바 스레드

### 플랫폼 스레드

<img width="508" alt="스크린샷 2025-02-26 오후 11 41 09" src="https://github.com/user-attachments/assets/5052df43-288b-4c86-8c7a-01ef5e392d5a" />


핵심 메소드가 전부 네이티브 코드로 구현 (JNI 사용)
>플랫폼 스레드는 커널 스레드의 지원을 받아 1대1 관계로 존재하기 때문(일대일 스레딩 모델)

플랫폼 스레드의 단점
1. 커널 스레드를 기반으로 구현되기 때문에 생성, 소멸, 동기화 등 다양한 **스레드 연산이 유저 모드에서 커널 모드로 전환하는 시스템 콜로 이루어지기 때문에 비용이 큼**

유저모드 -> 커널모드 전환 비용(컨텍스트 스위칭 비용): TCB(레지스터 값) 저장
>이때 커널모드->유저 모드로 다시 전환될 때 
>같은 프로세스의 스레드: TLB 무효화, 캐시 무효화 필요 X (가상 주소 공간이 같으므로)
>
>다른 프로세스의 스레드: TLB 무효화, 캐시 무효화 필요 O (가상 주소 공간이 다르므로)


2. 커널 스레드에 매핑되기 때문에 일정량의 커널 자원을 소모

### 사용자 스레드(ULT)

<img width="352" alt="스크린샷 2025-02-27 오전 12 27 01" src="https://github.com/user-attachments/assets/c1f9371d-83bb-412d-8704-a8a814067253" />


1:N 구현
온전히 유저 공간에서 구현되는 스레드로 커널의 도움 없이 유저 모드에서 처리되기 때문에 커널 모드 전환 비용이 없다는 장점

다만 스레드의 생성, 소멸, 동기화, 스케줄링 등 모든 작업을 사용자 프로그램 자체에서 처리해야 함
>잘못 다루면 프로세스 전체가 블로킹 될 수 있음

## 하이브리드 구현 (커널 스레드 + 사용자 스레드)

<img width="460" alt="스크린샷 2025-02-27 오전 12 25 51" src="https://github.com/user-attachments/assets/00f20196-bcbe-4443-93cc-4ea46c6310f9" />


플랫폼 스레드가 사용자 스레드와 커널 스레드 사이에서 가교 역할

여기서 사용자 스레드가 java21에 등장한 Virtual Thread

## 번외: 내가 이해한 Spring MVC vs Spring Webflux

Spring MVC는 요청이 들어오면 즉시 워커 스레드가 할당되어 작업을 처리하는 반면, 

WebFlux는 우선 이벤트 루프 스레드에서 작업을 처리하다가 블로킹 작업이 예상되면 필요한 경우에만 워커 스레드로 위임하는 **“lazy”** 한 전략
>Spring MVC는 thread per request인거지 thread per connection은 아님
